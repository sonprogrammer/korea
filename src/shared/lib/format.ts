...
function getSafeDate(dateInput?: string | Date): Date {
...
export function formatDateKST(isoString: string): string {
  return format(getSafeDate(isoString), "yyyy-MM-dd");
}


//  * 4. ISO 스트링을 KST 기준 "HH:mm:ss"로 변환
export function formatTimeKST(isoString: string): string {
  return format(getSafeDate(isoString), "HH:mm:ss");
}
...
export function formatChartDate(dateStr: string): string {
  ...
  try {
    const kstDate = getSafeDate(dateStr);
    return format(kstDate, "M/d");
  } catch (error) {
    console.error("❌ [formatChartDate] 변환 에러:", error);
    return "";
  }
}
